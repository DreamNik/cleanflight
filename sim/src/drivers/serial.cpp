...
serialPort_t* uartOpen( USART_TypeDef* USARTx , serialReceiveCallbackPtr callback , uint32_t baudRate , portMode_t mode , portOptions_t options ){
	...
	p->cfg.vTable     = &p->vtable;
	
	p->vtable.serialWrite = []( serialPort_t* instance , uint8_t ch ){
		PortThread* p = find(instance);
		p->send_buf.push( ch );
	};
	
	p->vtable.serialTotalRxWaiting = []( serialPort_t* instance ) -> uint8_t {
		PortThread* p = find(instance);
		return std::min( p->recv_buf.length() , 255 );
	};

	p->vtable.serialTotalTxFree = []( serialPort_t* instance ) -> uint8_t {
		UNUSED(instance);
		return 127;
	};
	
	p->vtable.serialRead = []( serialPort_t* instance ) -> uint8_t{
		PortThread* p = find(instance);
		char r = 0;
		p->recv_buf.pop( &r );
		return r;
	...
	p->vtable.serialSetBaudRate = []( serialPort_t* instance , uint32_t baudRate ){
		PortThread* p = find(instance);
	...
	p->vtable.setMode = []( serialPort_t* instance , portMode_t mode ){
		UNUSED( instance );
		UNUSED( mode );
	};

	p->vtable.isSerialTransmitBufferEmpty = []( serialPort_t* instance ){
		UNUSED(instance);
		return true;
	};

	p->vtable.writeBuf = [](serialPort_t *instance, void *data, int count){
		PortThread* p = find(instance);
		for( int c=0 ; c < count ; c++ ){
			p->send_buf.push( ((uint8_t*)data)[c] );
		}
	};
	
	
	p->vtable.beginWrite = [](serialPort_t *instance){
		UNUSED(instance);
	};

	p->vtable.endWrite = [](serialPort_t *instance){
		UNUSED(instance);
	};
